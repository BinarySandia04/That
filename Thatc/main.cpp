...
#include <That/logs.h>
#include <That/Utils/system.h>
#include <That/Utils/thatpath.h>
...
int main(int argc, char *argv[]) {
  ...
  if (cmdl[1] == "init" || cmdl[1] == "i") {
    ...
    if (cmdl[2] != "") {
      try {
        Package *pack = FetchPackage(cmdl[2]);
    ...
    VM vm = VM();
    std::string data;
    if(readFile(cmdl[2].c_str(), &data)){
      Logs::Error("Error loading script " + cmdl[2]);
      return 0;
    }
    vm.runCode(data);
    return 0;
